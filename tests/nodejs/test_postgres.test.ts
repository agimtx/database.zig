...
function buildPostgresTypeCoverageCase(tableName: string): TypeCoverageCase {
  return {
...
function assertPostgresTypeCoverageValues(resultSet: ResultSet): void {
  ...
  const timeValue = resultSet.value(0, 8);
  assertNonEmptyValue(timeValue, "time_value");
  assert.match(timeValue, /^03:04:05(?:\.\d+)?$/);
  const intervalValue = resultSet.value(0, 9);
  assertNonEmptyValue(intervalValue, "interval_value");
  assert.match(intervalValue, /^P0M1DT00:00:02(?:\.0+)?$/);
  ...
  const timestampValue = resultSet.value(0, 14);
  ...
  if (columns[15].columnType === bindingModule.COLUMN_TYPES.JSON) {
    ...
    const jsonValue = resultSet.value(0, 15);
...
async function assertPostgresAdditionalTypeCoverage(connection: Connection): Promise<void> {
  ...
  try {
    ...
    for (const [index, label] of [[1, "bit_value"], [2, "varbit_value"], [11, "timetz_value"]] as const) {
  ...
  await assert.rejects(
    ...
    (error) => assertErrorMessage(error, /internal error/i),
  );
}

async function runPostgresLifecycleTest(): Promise<void> {
  if ("skip" in target) {
