...
const target = loadTarget("databricks");

const skipReason = bindingLoadError
  ? `node binding dependencies are not available: ${bindingLoadError.message}`
  : !shouldRunSection("databricks")
    ? "DATABASE_ZIG_TEST_SECTION is filtering out databricks"
    : "skip" in target
      ? target.skip
      : undefined;

test("test_databricks", { skip: skipReason }, async () => {
  if ("skip" in target) {
