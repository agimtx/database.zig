...
const target = loadTarget("snowflake");

const skipReason = bindingLoadError
  ? `node binding dependencies are not available: ${bindingLoadError.message}`
  : !shouldRunSection("snowflake")
    ? "DATABASE_ZIG_TEST_SECTION is filtering out snowflake"
    : "skip" in target
      ? target.skip
      : undefined;

test("test_snowflake", { skip: skipReason }, async () => {
  if ("skip" in target) {
