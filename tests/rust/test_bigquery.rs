...
#[test]
fn rust_binding_bigquery_catalogs() {
    if let Some(reason) = maybe_skip_configured_section("bigquery") {
        eprintln!("skipping rust BigQuery integration test: {reason}");
        return;
    }

    let target = load_target("bigquery").expect("bigquery target should load after skip check");
    let test_result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let connection = match manager.connect(target.driver, &target.dsn(None)) {
            ...
            Err(error) if is_runtime_unavailable(&error) => {
                eprintln!("skipping rust BigQuery integration test at runtime: {error}");
                return Ok(());
            }
