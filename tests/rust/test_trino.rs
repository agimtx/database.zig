...
#[test]
fn rust_binding_trino_catalogs() {
    if let Some(reason) = maybe_skip_configured_section("trino") {
        eprintln!("skipping rust Trino integration test: {reason}");
        return;
    }

    let target = load_target("trino").expect("trino target should load after skip check");
    let test_result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let connection = match manager.connect(target.driver, &target.dsn(None)) {
            ...
            Err(error) if is_runtime_unavailable(&error) => {
                eprintln!("skipping rust Trino integration test at runtime: {error}");
                return Ok(());
            }
