...
#[test]
fn rust_binding_mssql_catalogs() {
    if let Some(reason) = maybe_skip_configured_section("mssql") {
        eprintln!("skipping rust MSSQL integration test: {reason}");
        return;
    }

    let target = load_target("mssql").expect("mssql target should load after skip check");
    let test_result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let connection = match manager.connect(target.driver, &target.dsn(None)) {
            ...
            Err(error) if is_runtime_unavailable(&error) => {
                eprintln!("skipping rust MSSQL integration test at runtime: {error}");
                return Ok(());
            }
