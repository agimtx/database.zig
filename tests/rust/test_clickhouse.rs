...
#[test]
fn rust_binding_clickhouse_catalogs() {
    if let Some(reason) = maybe_skip_configured_section("clickhouse") {
        eprintln!("skipping rust ClickHouse integration test: {reason}");
        return;
    }

    let target = load_target("clickhouse").expect("clickhouse target should load after skip check");
    let test_result = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let connection = match manager.connect(target.driver, &target.dsn(None)) {
            ...
            Err(error) if is_runtime_unavailable(&error) => {
                eprintln!("skipping rust ClickHouse integration test at runtime: {error}");
                return Ok(());
            }
        ...
        let catalogs_error = match connection.get_catalogs() {
            Ok(_) => panic!("clickhouse targets should reject get_catalogs"),
            Err(error) => error,
        };
