...
use support::{
    assert_columns, assert_table_qualified_name, duckdb_test_dsn, find_row_index, maybe_skip_duckdb,
    open_duckdb_connection, open_duckdb_manager, read_values, remove_file_if_exists, repo_tmp_dir,
    unique_identifier,
};
        ...
        let connection = match open_duckdb_connection(&manager, &dsn) {
            ...
            Err(error)
                if error.to_string().contains("Could not load")
                    ...
                    || error.to_string().contains("aq_connection_open failed:") =>
            {
