...
class TrinoBindingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_trino(self) -> None:
        if not should_run_section("trino"):
            self.skipTest("DATABASE_ZIG_TEST_SECTION is filtering out trino")

        target = load_test_target("trino")
