...
class SnowflakeBindingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_snowflake(self) -> None:
        if not should_run_section("snowflake"):
            self.skipTest("DATABASE_ZIG_TEST_SECTION is filtering out snowflake")

        target = load_test_target("snowflake")
