...
class ExasolBindingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_exasol(self) -> None:
        if not should_run_section("exasol"):
            self.skipTest("DATABASE_ZIG_TEST_SECTION is filtering out exasol")

        target = load_test_target("exasol")
