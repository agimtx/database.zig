...
class ClickHouseBindingIntegrationTest(unittest.IsolatedAsyncioTestCase):
    async def test_clickhouse(self) -> None:
        if not should_run_section("clickhouse"):
            self.skipTest("DATABASE_ZIG_TEST_SECTION is filtering out clickhouse")

        target = load_test_target("clickhouse")
