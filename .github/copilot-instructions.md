...
- This project is structured as Zig control plane + Apache Arrow ADBC layer + public C ABI + thin Python and Node.js bindings.
...
- New database support should be added by wiring an Apache Arrow ADBC driver into Zig rather than creating a new per-database binding surface.
- Arrow ADBC artifacts must be consumed by Zig through dynamic linking.
...
- Keep dependency flow one-way: `adbc shared libraries -> zig core -> public c abi -> language bindings`.
...
3. Zig-to-ADBC integration remains a dynamic shared-library dependency contract.
4. Language bindings only convert arguments and surface results.
5. The design still generalizes across multiple databases through ADBC configuration rather than separate public driver kinds.